  ...
  async processChat(dto: ChatQueryDto) {
    ...
    if (this.geminiApiKey) {
      try {
        ...
        this.logger.warn(
          `Gemini API call failed: ${(err as Error).message}. Falling back to semantic engine.`
        )
      }
    }
...
• **Perks**: 1-on-1 Investor Deck Review, priority slot in Investor Open Hours, and Startup Expo booth discounts.`
    ...
    if (
      ...
      q.includes('speaking') ||
      q.includes('guest') ||
      q.includes('who is coming')
    ) {
      const speakerList = context.speakers
        .slice(0, 5)
        .map((s) => `• **${s.name}** (${s.title}): ${s.bio}`)
        .join('\n')
    ...
    if (
      ...
      const day1Text = day1
        .map((e) => `  - ${e.startTime}–${e.endTime}: **${e.title}** (${e.venue})`)
        .join('\n')
      const day2Text = day2
        .map((e) => `  - ${e.startTime}–${e.endTime}: **${e.title}** (${e.venue})`)
        .join('\n')
...
What would you like to explore?`
  ...
  private async queryGemini(query: string, history: any[], context: any): Promise<string | null> {
